...
def extract_haralick_features_with_contrast(data_path, distances=[1, 5], gray_levels=256, contrast_factor=2.0):
    """
    Извлекает признаки Харалика для изображений в папках YES и NO с дополнительным повышением контрастности.
    ...
    contrast_factor (float): Фактор повышения контрастности (default=2.0)
    ...
    def process_image(image_path, distance, levels, contrast_factor):
        ...
        # Дополнительное повышение контрастности с PIL
        pil_image = Image.fromarray(cv2.cvtColor(processed_image, cv2.COLOR_BGR2RGB))
        enhancer = ImageEnhance.Contrast(pil_image)
        enhanced_image = enhancer.enhance(contrast_factor)
        processed_image = cv2.cvtColor(np.array(enhanced_image), cv2.COLOR_RGB2BGR)
    ...
    for filename in os.listdir(yes_path):
        if filename.endswith('.bmp'):
            ...
            for d in distances:
                features = process_image(file_path, d, gray_levels, contrast_factor)
    ...
    # Сохраняем результаты в отдельные файлы с префиксом 'contrast_'
    for key, df in df_dict.items():
        filename = f'contrast_haralick_features_{key}.csv'
        df.to_csv(filename, index=False)

    print("Признаки Харалика с повышенной контрастностью успешно извлечены и сохранены в CSV файлы")
    return df_dict


# Пример использования
if __name__ == "__main__":
    data_path = '/medicine_ML/DATA'
    df_dict = extract_haralick_features_with_contrast(data_path, distances=[1, 5], gray_levels=256, contrast_factor=2.0)