...
def feature_selection_methods(X, y, k=13):  # Отбираем топ-13 из 26
    ...
    try:
        ...
        selector.fit(X, y)
        methods['t_test'] = X.columns[selector.get_support()]
    except ValueError as e:
        print(f"Ошибка в t_test: {e}")
        methods['t_test'] = X.columns[:k]

    # 2. Wilcoxon sum-rank test
    try:
        selector = SelectKBest(wilcoxon_score, k=k)
        selector.fit(X, y)
        methods['wilcoxon'] = X.columns[selector.get_support()]
    except ValueError as e:
        print(f"Ошибка в wilcoxon: {e}")
        methods['wilcoxon'] = X.columns[:k]

    # 3. Bhattacharyya distance
    try:
        selector = SelectKBest(bhattacharyya_distance, k=k)
        selector.fit(X, y)
        methods['bhattacharyya'] = X.columns[selector.get_support()]
    except ValueError as e:
        print(f"Ошибка в bhattacharyya: {e}")
        methods['bhattacharyya'] = X.columns[:k]

    # 4. Relative entropy (KL divergence)
    try:
        selector = SelectKBest(relative_entropy, k=k)
        selector.fit(X, y)
        methods['relative_entropy'] = X.columns[selector.get_support()]
    except ValueError as e:
        print(f"Ошибка в relative_entropy: {e}")
        methods['relative_entropy'] = X.columns[:k]
    ...
    try:
        ...
        selector.fit(X, y)
        methods['svm_rfe'] = X.columns[selector.support_]
    except ValueError as e:
        print(f"Ошибка в svm_rfe: {e}")
        methods['svm_rfe'] = X.columns[:k]
...
def run_feature_selection_and_classification(data_path):
    ...
    all_results = {}
    for method, cols in selected_features.items():
        print(f"\n=== Оценка для метода отбора: {method} (отобрано {len(cols)} признаков) ===")
        print(f"Выбранные признаки: {list(cols)}")
        X_selected = X[cols]
